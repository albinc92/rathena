 ...
 * @param itm Item data
 **/
void mob_setdropitem_option2(struct item *itm, int magic_find) {
...
t_itemid set_drop_id(int mob_level, e_mob_bosstype boss_type) {
	...
	if(!is_rare) { // NORMAL ITEMS
		...
		if(item_type < 10) { // ARMOR
		...
		else if(item_type > 20 && item_type < 54) { // ACCESSORY
		...
		else if(item_type > 74 && item_type < 78) { // 1-H SPEAR (ilv based on highest weapon atk (180 in this case) / 100)
		...
		else if(item_type > 86 && item_type < 91) { // 2-H AXE
		  ...
		  id_range.push_back({1705, 28});
		  id_range.push_back({1708, 31});
		  id_range.push_back({1711, 33});
		  id_range.push_back({1715, 52});
		  id_range.push_back({1716, 60});
		  ...
		  id_range.push_back({lv_30_upper_head.at(rnd() % lv_30_upper_head.size()), 30});
		  id_range.push_back({5118, 33});
		  std::vector<t_itemid> lv_34_upper_head = { 2279, 2282 };
		  id_range.push_back({lv_34_upper_head.at(rnd() % lv_34_upper_head.size()), 34});
		...
		else if(item_type > 170 && item_type < 177) { // MIDDLE & LOWER HEADGEAR
		...
		else if(item_type > 176 && item_type < 179) { // UPPER & MIDDLE & LOWER HEADGEAR
				id_range.push_back({2264, 30});
				id_range.push_back({5046, 32});
			}
		else if(item_type > 178 && item_type < 184) { // 2H-STAFF (no rare/normal types)
		...
		else if(item_type > 183 && item_type < 187) { // SHOTGUN (no rare/normal types)
	  ...
	  else if(item_type > 33 && item_type < 53) { // GARMENT
    ...
    else if(item_type > 52 && item_type < 68) { // FOOTGEAR
    ...
    else if(item_type > 67 && item_type < 92) { // DAGGER (iLvl based on mob lvl that usually drops)
    ...
    else if(item_type > 91 && item_type < 104) { // 1-H SWORD
    ...
    else if(item_type > 103 && item_type < 114) { // 2-H SWORD
    ...
    else if(item_type > 113 && item_type < 120) { // 1-H SPEAR (ilv based on highest weapon atk (180 in this case) / 100)
    ...
    else if(item_type > 119 && item_type < 128) { // 2-H SPEAR (ilv based on highest weapon atk (200 in this case) / 100)
		...
		else if(item_type > 127 && item_type < 131) { // 1-H AXE
			id_range.push_back({1307, 66});
			id_range.push_back({1306, 70});
			id_range.push_back({1311, 83});
		}
		else if(item_type > 130 && item_type < 141) { // 2-H AXE
    ...
    else if(item_type > 140 && item_type < 151) { // MACE
		...
		else if(item_type > 150 && item_type < 162) { // 1H-STAFF
		...
		else if(item_type > 161 && item_type < 172) { // BOW
    ...
    else if(item_type > 171 && item_type < 176) { // KNUCKLE
    ...
    else if(item_type > 175 && item_type < 182) { // MUSICAL INSTRUMENT
    ...
    else if(item_type > 181 && item_type < 192) { // WHIP
    ...
    else if(item_type > 191 && item_type < 202) { // BOOK
    ...
    else if(item_type > 201 && item_type < 214) { // KATAR
	    ...
	    id_range.push_back({lv_25_upper_head_r.at(rnd() % lv_25_upper_head_r.size()), 25});
	    id_range.push_back({2253, 26});
	    std::vector<t_itemid> lv_31_upper_head_r = { 2206, 2285 };
	    id_range.push_back({lv_31_upper_head_r.at(rnd() % lv_31_upper_head_r.size()), 31});
	    ...
	    id_range.push_back({lv_51_upper_head_r.at(rnd() % lv_51_upper_head_r.size()), 51});
    ...
    else if(item_type > 257 && item_type < 260) { // UPPER & MIDDLE HEADGEAR
			id_range.push_back({5093, 80});
	    id_range.push_back({5022, 99});
		}
    else if(item_type > 259 && item_type < 261) { // MIDDLE & LOWER HEADGEAR
			id_range.push_back({5005, 38});
		}
    else if(item_type > 260 && item_type < 262) { // UPPER & MIDDLE & LOWER HEADGEAR
			id_range.push_back({5808, 99});
		}
		else if(item_type > 261 && item_type < 295) { // ACCESSORY (no rare/normal types. ilv based on moblv who normally drops)
			id_range.push_back({2609, 10});
		  std::vector<t_itemid> lv_31_accessory = { 2602, 2618 };
		  id_range.push_back({lv_31_accessory.at(rnd() % lv_31_accessory.size()), 31});
		  std::vector<t_itemid> lv_37_accessory = { 2611, 2604 };
		...
		else if(item_type > 294 && item_type < 300) { // 2H-STAFF (no rare/normal types)
			id_range.push_back({1473, 64});
		  id_range.push_back({1472, 75});
		  id_range.push_back({2004, 80});
		  std::vector<t_itemid> lv_81_2hstaff_r = { 2005, 2001 };
		...
		else if(item_type > 299 && item_type < 303) { // SHOTGUN (no rare/normal types)
			id_range.push_back({13167, 79});
		  id_range.push_back({13156, 82});
		  id_range.push_back({13169, 89});
		}
 ...
 *------------------------------------------*/
static struct item_drop* mob_setdropitem(struct s_mob_drop *mobdrop, int qty, unsigned short mob_id, int magic_find)
{
...
int mob_dead(struct mob_data *md, struct block_list *src, int type)
{
	...
	{ // Item Drop
		...
		for (i = 0; i < MAX_MOB_DROP_TOTAL; i++) {
			...
			ditem = mob_setdropitem(&md->db->dropitem[i], 1, md->mob_id, sd->bonus.magic_find);
		...
		if (sd == mvp_sd && pc_checkskill(sd,BS_FINDINGORE)>0 && battle_config.finding_ore_rate/10 >= rnd()%10000) {
			...
			ditem = mob_setdropitem(&mobdrop, 1, md->mob_id, sd->bonus.magic_find);
		...
		if(sd) {
			...
			for (const auto &it : sd->add_drop) {
					...
					(it.class_ == CLASS_ALL || it.class_ == status->class_)) //Matched class
				{
					...
					mob_item_drop(md, dlist, mob_setdropitem(&mobdrop,1,md->mob_id,sd->bonus.magic_find), 0, drop_rate, homkillonly);
				}
			}
